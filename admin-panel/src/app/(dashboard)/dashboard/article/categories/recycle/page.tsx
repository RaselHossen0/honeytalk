...
import {
  fetchCategoryRecycleBin,
  restoreCategory,
  batchRestoreCategories,
  permanentDeleteCategory,
  batchPermanentDeleteCategories,
} from '@/services/article-category';

export default function CategoryRecycleBinPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetchCategoryRecycleBin(page + 1, perPage);
      setData(res.data);
      setTotal(res.total);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    addTab({
      id: '/dashboard/article/categories/recycle',
      label: 'Category Management Recycle Bin',
      path: '/dashboard/article/categories/recycle',
      breadcrumbs: ['Home', 'Article Management', 'Category Management Recycle Bin'],
    });
  }, [addTab]);
  ...
  const handleRestore = async (row: ArticleCategory) => {
    await restoreCategory(row.id);
    load();
  };

  const handleDelete = async (row: ArticleCategory) => {
    if (!confirm(`Permanently delete "${row.categoryName}"?`)) return;
  ...
  const handleBatchRestore = async () => {
    ...
    await batchRestoreCategories(selected);
    setSelected([]);
    load();
  };
  ...
  const handleBatchDelete = async () => {
    ...
    await batchPermanentDeleteCategories(selected);
    setSelected([]);
    load();
  };
