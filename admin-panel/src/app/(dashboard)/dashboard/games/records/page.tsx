...
export default function GameRecordsPage() {
  ...
  const [data, setData] = useState<Awaited<ReturnType<typeof fetchGameRecords>>['data']>([]);
  ...
  const [filters, setFilters] = useState<GameRecordFilters>({
    type: 'all',
    identification: '',
    timeStart: '',
    timeEnd: '',
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const hasFilters = filters.identification || filters.timeStart || filters.timeEnd || (filters.type && filters.type !== 'all');
      const res = await fetchGameRecords(hasFilters ? filters : undefined, page + 1, perPage);
      setData(res.data);
      setTotal(res.total);
      setConsumptionAmount(res.consumptionAmount);
      setRewardAmount(res.rewardAmount);
      setPlatformRevenue(res.platformRevenue);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    addTab({
      id: '/dashboard/games/records',
      label: 'Game Records',
      path: '/dashboard/games/records',
      breadcrumbs: ['Home', 'Plugin management', 'Third party games', 'Game Records'],
    });
  }, [addTab]);
  ...
  const handleClear = () => {
    setFilters({ type: 'all', identification: '', timeStart: '', timeEnd: '' });
    setPage(0);
  };
  ...
  return (
    <Box>
      ...
      <Box sx={{ mb: 2, display: 'flex', gap: 3, flexWrap: 'wrap' }}>
        ...
        <Typography variant="body1">
          <strong>Platform revenue:</strong> {platformRevenue.toLocaleString()}
        </Typography>
      </Box>
        ...
        <Table size="small">
          <TableHead>
            <TableRow sx={{ bgcolor: 'grey.50' }}>
              ...
              <TableCell sx={{ fontWeight: 600 }} align="right">Platform revenue</TableCell>
            ...
            {loading ? (
              ...
              data.map((row) => (
                <TableRow key={row.id} hover sx={{ '&:nth-of-type(even)': { bgcolor: 'grey.50' } }}>
                  ...
                  <TableCell align="right">{row.platformRevenue.toLocaleString()}</TableCell>
