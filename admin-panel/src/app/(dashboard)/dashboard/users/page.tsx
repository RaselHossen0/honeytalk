...
function truncateId(id: string | undefined, max = 16): string {
  if (!id) return '—';
    ...
    <Box sx={{ py: 2 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2, mb: 3 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>Search, filter, and manage user accounts</Typography>
        </Box>
            ...
            <TableHead>
              <TableRow sx={{ '& th': { bgcolor: 'grey.50', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.5, color: 'text.secondary', borderBottom: '1px solid', borderColor: 'divider', py: 1.25 } }}>
                {tableHeaders.map((h, i) => (
                  <TableCell
                    key={h}
                    align={h.includes('Diamonds') || h.includes('amount') || h.includes('balance') || h.includes('Accumulated') || h.includes('Points') || h.includes('Follow') || h.includes('Fans') || h.includes('viewers') ? 'right' : 'left'}
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={34} align="center" sx={{ py: 8 }}>
                    ...
                    <Typography component="span" variant="body2" color="text.secondary">Loading users...</Typography>
                  </TableCell>
                </TableRow>
              ) : data.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={34} align="center" sx={{ py: 8 }}>
                    <Typography variant="body2" color="text.secondary">No users found. Adjust your filters or add a new user.</Typography>
                  </TableCell>
                </TableRow>
              ) : (
              data.map((row) => (
                ...
                >
                  ...
                  <TableCell align="right">{(row.coinAccumulated ?? 0).toFixed(2)}</TableCell>
                  <TableCell>
                    <Link href="#" color="primary" sx={{ fontSize: 12, cursor: 'pointer' }} onClick={(e) => e.preventDefault()}>
                      {row.recommended ? 'Yes' : 'No'}
                    </Link>
                  </TableCell>
                  <TableCell>{row.certification ?? 'Not verified'}</TableCell>
                  <TableCell align="right">{row.points ?? 0}</TableCell>
                  <TableCell align="right">{row.follow ?? 0}</TableCell>
                  <TableCell align="right">{row.fans ?? 0}</TableCell>
                  <TableCell>{row.level ?? 'Lv1'}</TableCell>
                  <TableCell>
                    <Box component="span" sx={{ color: 'warning.main' }}>{row.permanentBan ? 'Yes' : 'No'}</Box>
                  </TableCell>
                  <TableCell>{row.fancyNumber ?? '—'}</TableCell>
                  <TableCell align="right">{row.totalViewers ?? 0}</TableCell>
                  <TableCell>
                    <Box>
                      {row.registerIp || '—'}
                      {row.registerIp && (
                        <Box component="span" sx={{ display: 'block' }}>
                          <Link href="#" color="primary" sx={{ fontSize: 12, cursor: 'pointer' }} onClick={(e) => e.preventDefault()}>Ban</Link>
                        </Box>
                      )}
                    </Box>
                  </TableCell>
                  ...
                  <TableCell>
                    {row.googleId ? (
                      <Box>
                        <Tooltip title={row.googleId} placement="top" arrow>
                          <Box component="span" sx={{ fontFamily: 'monospace', fontSize: 12 }}>{truncateId(row.googleId)}</Box>
                        </Tooltip>
                        <Box sx={{ mt: 0.25 }}>
                          <Link href="#" color="primary" sx={{ fontSize: 11, cursor: 'pointer' }} onClick={(e) => e.preventDefault()}>Ban</Link>
                        </Box>
                      </Box>
                    ) : '—'}
                  </TableCell>
                  <TableCell>
                    {row.appleId ? (
                      <Box>
                        <Tooltip title={row.appleId} placement="top" arrow>
                          <Box component="span" sx={{ fontFamily: 'monospace', fontSize: 12 }}>{truncateId(row.appleId)}</Box>
                        </Tooltip>
                        <Box sx={{ mt: 0.25 }}>
                          <Link href="#" color="primary" sx={{ fontSize: 11, cursor: 'pointer' }} onClick={(e) => e.preventDefault()}>Ban</Link>
                        </Box>
                  ...
                  <TableCell>
                    <Link
                      ...
                      onClick={(e) => { e.preventDefault(); openAction('statusConfirm', row); }}
                    >
                      {row.status ?? 'Valid'}
                    </Link>
                  </TableCell>
                  <TableCell>{row.isVip ? 'Yes' : 'No'}</TableCell>
                  <TableCell>
                    <Link href="#" color="primary" sx={{ fontSize: 12, cursor: 'pointer' }} onClick={(e) => e.preventDefault()}>
                      {row.disablePopular ? 'Yes' : 'No'}
                    </Link>
                  </TableCell>
                  <TableCell>
                    <Box component="span" sx={{ color: 'warning.main' }}>
                      <Link href="#" sx={{ fontSize: 12, cursor: 'pointer', color: 'inherit' }} onClick={(e) => e.preventDefault()}>
                        {row.globallyMuteIm ? 'Yes' : 'No'}
                      </Link>
                    </Box>
                  </TableCell>
                  <TableCell>{row.loginMethod ?? '—'}</TableCell>
