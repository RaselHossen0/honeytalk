...
import { Search, Clear, CheckCircle, Cancel, Person } from '@mui/icons-material';
...
export default function WithdrawalPendingReviewPage() {
  const addTab = useTabsStore((s) => s.addTab);
  const [data, setData] = useState<Withdrawal[]>(() =>
    demoWithdrawals.filter((r) => r.status === 'Pending Review' || r.status === 'Pending')
  );
  ...
  useEffect(() => {
    addTab({
      id: '/dashboard/fund/withdrawal/pending-review',
      label: 'Withdrawal Pending Review Record',
      path: '/dashboard/fund/withdrawal/pending-review',
      breadcrumbs: ['Home', 'Fund Management', 'Withdrawal management', 'Withdrawal Pending Review Record'],
    });
  }, [addTab]);
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.status && filters.status !== 'Select') {
      ...
      result = result.filter((r) => r.status === mapped);
    }
    return result;
  }, [data, filters]);
  ...
  const handleClear = () => {
    setFilters({ anchorId: '', anchorNickname: '', status: 'Pending review' });
    setPage(0);
  };
      ...
      <Paper sx={{ width: '100%', overflow: 'hidden', borderRadius: 2 }}>
          ...
          <Table size="small" stickyHeader>
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 600, bgcolor: 'grey.50', minWidth: 130 }}>Bank Account Number</TableCell>
                ...
                <TableCell sx={{ fontWeight: 600, bgcolor: 'grey.50', minWidth: 115 }}>Whether to review</TableCell>
                <TableCell sx={{ fontWeight: 600, bgcolor: 'grey.50', minWidth: 160 }}>Confirm payment time</TableCell>
                <TableCell sx={{ fontWeight: 600, bgcolor: 'grey.50', minWidth: 110 }}>Operation notes</TableCell>
        ...
        {selected.length > 0 && (
          <Box sx={{ p: 2, borderTop: 1, borderColor: 'divider' }}>
            <Button variant="contained" startIcon={<Person />} onClick={handleBatchReview}>
              Batch review
            </Button>
          </Box>
        )}
