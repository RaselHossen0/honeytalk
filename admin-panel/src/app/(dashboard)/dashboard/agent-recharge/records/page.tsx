...
import type { AgentRechargeRecord } from '@/types/agent-recharge';
import { demoAgentRechargeRecords } from '@/lib/demo-data';

export default function RechargeRecordPage() {
  const addTab = useTabsStore((s) => s.addTab);
  const [data] = useState<AgentRechargeRecord[]>(demoAgentRechargeRecords);
  ...
  useEffect(() => {
    addTab({
      id: '/dashboard/agent-recharge/records',
      label: 'Recharge record',
      path: '/dashboard/agent-recharge/records',
      breadcrumbs: ['Home', 'Agent Recharge Management', 'Recharge record'],
    });
  }, [addTab]);
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.agentName.trim()) {
      const k = filters.agentName.toLowerCase();
      result = result.filter((r) => r.agentName.toLowerCase().includes(k));
    }
  ...
  const rechargeStatistics = useMemo(() => filtered.reduce((sum, r) => sum + r.receivingAccount, 0), [filtered]);
      ...
      <Paper sx={{ p: 2, mb: 2, borderRadius: 2 }}>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, alignItems: 'flex-end' }}>
          <FormControl size="small" sx={{ minWidth: 180 }}>
            <InputLabel>Agent Account ManagementID</InputLabel>
            <Select value={filters.agentId} label="Agent Account ManagementID" onChange={(e) => setFilters((p) => ({ ...p, agentId: e.target.value }))}>
          ...
          <TextField size="small" label="Agent Account Managemen" placeholder="Agent name" value={filters.agentName} onChange={(e) => setFilters((p) => ({ ...p, agentName: e.target.value }))} sx={{ minWidth: 160 }} />
          <TextField size="small" label="User nickname" placeholder="Please enter user r" value={filters.userNickname} onChange={(e) => setFilters((p) => ({ ...p, userNickname: e.target.value }))} sx={{ minWidth: 160 }} />
          <TextField size="small" label="Superior ID" value={filters.superiorId} onChange={(e) => setFilters((p) => ({ ...p, superiorId: e.target.value }))} sx={{ minWidth: 120 }} />
          <TextField size="small" label="Superior" value={filters.superiorName} onChange={(e) => setFilters((p) => ({ ...p, superiorName: e.target.value }))} sx={{ minWidth: 120 }} />
          <TextField size="small" label="Operator ID" value={filters.operatorId} onChange={(e) => setFilters((p) => ({ ...p, operatorId: e.target.value }))} sx={{ minWidth: 120 }} />
          <TextField size="small" label="Operator" value={filters.operator} onChange={(e) => setFilters((p) => ({ ...p, operator: e.target.value }))} sx={{ minWidth: 120 }} />
